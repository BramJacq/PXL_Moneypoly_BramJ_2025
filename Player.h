...
public:
    // default ctor
    Player() : name_("Player"), money_(500), pos_(0), human_(false) {}

    // parameterized ctor (name, money optional, human flag)
    Player(const std::string &name, int money = 500, bool human = false)
        : name_(name), money_(money), pos_(0), human_(human) {}

    // copy ctor
    Player(const Player &other)
        : name_(other.name_), money_(other.money_), pos_(other.pos_), human_(other.human_) {}

    // destructor
    ~Player() = default;

    // movement using unsigned char (memory efficient small type)
    inline void move(unsigned char steps, unsigned char boardSize) {
        pos_ = static_cast<unsigned char>((pos_ + steps) % boardSize);
    }

    inline void changeMoney(int delta) { this->money_ += delta; } // useful "this"
    inline bool canBuy(int cost) const { return money_ >= cost; }

    inline const std::string &getName() const { return name_; } // const ref
